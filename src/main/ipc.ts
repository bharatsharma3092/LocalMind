    ...
    ;(async () => {
      ...
      try {
        ...
        if (!doneSent && !controller.signal.aborted && !browserWin.isDestroyed()) {
          ...
          log.info('startStream', 'Stream completed normally -- done already sent, skipping fallback', { streamId, chunkCount })
        } else {
          log.info('startStream', 'Stream ended (aborted or window destroyed) -- no done sent', { streamId })
        }
